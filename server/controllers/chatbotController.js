...
if (!process.env.GROQ_API_KEY) {
    console.error("CRITICAL: GROQ_API_KEY is NOT set in environment!");
} else {
    console.log("GROQ_API_KEY is loaded (length: " + process.env.GROQ_API_KEY.length + ")");
}
...
export const createChat = async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({
            ...
            alertMessage: sentimentScore > 0.7 ? "We've noticed you might be going through a tough time. Remember, NeuroSentinel is here for you, but please consider reaching out to a professional or a loved one if you feel overwhelmed." : null,
            newlyAwarded
        });
...
export const updateChat = async (req, res) => {
    try {
        ...
        try {
            ...
            // Build messages array for Groq with system prompt first, then conversation history
            const messages = [
                {
                    role: "system",
                    content: CBT_SYSTEM_PROMPT
            ...
            // Add previous conversation history (excluding the current question being processed)
            const history = (chat.history || []);
            for (const item of history) {
                const role = item.role === "model" ? "assistant" : "user";
                const text = item.parts && item.parts[0] ? decrypt(item.parts[0].text) : "";
            ...
            log(`[updateChat] Sending ${messages.length - 1} messages to Groq (including system prompt).`);
